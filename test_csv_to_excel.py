...
# ======================================================================
# 17. ИДЕНТИФИКАТОР - НЕ ВЕЛИЧИНА
# Найдено 18.09.2026, после того как тот же дефект вылез на живом API.
# Почтовый индекс 05401 становился числом 5401 - ведущий ноль исчезал
# навсегда; телефон 8025285988 становился 8,025,285,988, то есть
# денежным форматом на телефоне; штрихкод - дробным числом.
# Ничего не падало и ничего не сообщалось.
# ======================================================================
print("\n-- 17. идентификаторы -----------------------------------------")

for _n, _v in (("телефоны", ["8025285988", "8028575318", "8022536708",
                             "8024647702"]),
               ("штрихкоды", ["4601234567890", "4609876543210",
                              "4601112223334"]),
               ("индекс с ведущим нулём", ["05401", "05672", "05344",
                                           "05452"]),
               ("артикул 007", ["007", "014", "021", "028"])):
    _c, _ = M.coerce_numeric(pd.Series(_v))
    check(f"REGRESSION остался текстом: {_n}", _c is None, True)

for _n, _v in (("суммы", ["1234.56", "987.80", "24.99"]),
               ("население разной длины", ["1234567", "89012", "450123456",
                                           "77"]),
               ("количество", ["1", "2", "3", "4"]),
               ("год", ["2024", "2025", "2026"]),
               ("деньги с разделителем", ["1 234,56", "9 876,00",
                                          "12 345,70"])):
    _c, _ = M.coerce_numeric(pd.Series(_v))
    check(f"по-прежнему число: {_n}", _c is not None, True)
